...
router.get('/login', (req, res) => {
	res.render('login', {login: req.session.verifyLogin});
});
	...
	if(username && password){
		...
		con.query("SELECT * FROM USER WHERE email_id = ?", [username], (err, row) =>{
			...
			if (row.length>0 && bcrypt.compareSync(password,row[0].password)){
				req.session.verifyLogin = true
				req.session.username = username;
				res.redirect('/user/');
			}else{
				res.render('login', {er:"Incorrect Username or Password", login: req.session.verifyLogin});
			}
		});
	}else{
		res.render('login', {er: "Enter Username and Password", login:req.session.verifyLogin});
	}
});
...
router.get('/register', (req,res) => {
	res.render('register',{login: req.session.verifyLogin});
});
...
router.get('/admin-login', (req,res)=>{
	res.render('admin_login',{login:req.session.verifyLogin});
});
...
router.post('/admin-login', (req,res)=>{
	...
	if(username=="admin@gmail.com" && password=="admin123")
	{
		...
		res.render('admin_login',{er:error, login:req.session.verifyLogin});
	}
});
...
router.get('/about', (req,res)=>{
	if(req.session.verifyLogin) res.redirect('/user/about');

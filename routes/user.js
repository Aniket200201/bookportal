...
router.get('/', verifyLogin, (req, res) => {
		con.query('select * from BOOK where owner<>(select user_id from USER where email_id=?)',[req.session.username], (err, row) => {
			if(err) throw err;
			res.render('home', {book: row, login: req.session.verifyLogin});
	});
	
});

router.get('/addBook',verifyLogin, (req,res) => {
	res.render('addbook', {login : req.session.verifyLogin});
});
...
router.post('/addBook', (req,res) => {
	...
	if(description.length<500)
	{
		...
		res.render('addbook', {msg:'Description max length is 500.', login : req.session.verifyLogin})
	}
...
router.get('/dashboard',verifyLogin, (req, res) => {
	...
	con.query('select * from USER where email_id = ?', [req.session.username], (err, result) => {
		if (err) throw err;
		res.render('dashboard', {content:content, user:result[0], login: req.session.verifyLogin});
	});
});
...
router.get('/book', verifyLogin,  (req, res) => {
	...
	con.query('select * from BOOK where owner=(select user_id from USER where email_id=?)',[req.session.username], (er,row) => {
		if(er) throw er;
		res.render('mybooks', {book: row, login: req.session.verifyLogin});
	});
	

});

router.get('/bookdel',verifyLogin, (req,res)=>{
	var id = req.query.id;
		...
		con.query( "select * from REQUEST where status in ('Accepted','Not Viewed') and book=?",[id], (err, requ)=>{
			if(requ.length>0){
				...
				con.query('select * from USER where user_id=?',[requ[0].buyer], (error, buyer)=>{
					...
					res.render('bookdel', {book: book[0], login: req.session.verifyLogin, request:requ[0], accept:accept});
				});
			}else{
				res.render('bookdel', {book: book[0], login: req.session.verifyLogin, req:true});
			}
		});
...
router.get("/profile",verifyLogin, (req,res)=>{
	...
	con.query('select * from USER where user_id=?', [id], (err,user)=>{
		res.render('profile', {login: req.session.verifyLogin, user:user[0]});
	});
});
	...
	con.query('select user_id from USER where email_id=?',[req.session.username], (e,r)=>{
		...
		con.query('insert into SEARCH (user,book) values (?,?)', [r[0].user_id,id],(er,row)=>{
			...
			con.query("select * from BOOK where book_id = ?",[id], (er, row)=>{
				...
				res.render('bookdetail', {book: row[0], login: req.session.verifyLogin, status:status});
			});
		});
	})
...
router.get('/request',verifyLogin, (req,res) => {
	...
	con.query('select * from USER', (e,users)=>{
		...
		con.query('select * from REQUEST where buyer=?',[buyer.user_id], (er,reques)=>{
			con.query('select * from BOOK', (err, books)=>{
				...
				res.render('request',{login:req.session.verifyLogin, request: reques});
			});
		});
	});
});
...
router.get('/report',verifyLogin, (req,res)=>{
	res.render('report',{login: req.session.verifyLogin});
});
	...
	con.query('select * from USER where email_id=? or email_id=?',[email,req.session.username], (er,row)=>{
		if(row.length>1){
			...
			con.query('insert into REPORT (victim, reporter, reason, date) values (?,?,?,CURDATE())', report, (err,re)=>{
				res.render('report', {msg:'Complaint successfully sent to Admin' ,login:req.session.verifyLogin});
			});
		}
		else{
			res.render('report',{er:'User not Exists' ,login:req.session.verifyLogin});
		}
...
router.get('/recommendation', (req,res)=>{
	con.query('select * from BOOK', (er,books)=>{
		...
		con.query("select * from SEARCH where user=(select user_id from USER where email_id=?) order by id desc",[req.session.username], (er, row)=>{
			...
			pyshell.run('rs_fin_opt.py', options, (err,result)=>{
				if(err) throw err;
				...
				res.render('recommendation',{login:req.session.verifyLogin, book:book});*/
				console.log(content);
				var spawn = require("child_process").spawn;
				var process = spawn('python',["rs_fin_opt.py",content]);
				process.stdout.on('data', function(data)  {
					console.log(data.toString());
					res.send(data.toString());
				})
				//res.render('home',{login:req.session.verifyLogin, book:book});
    })
			});
...
router.get('/search',(req,res)=>{
	...
	con.query(sql, (error,row)=>{
		res.render('home',{login:req.session.verifyLogin, book:row});
	});
});
...
router.get('/about', (req,res)=>{
	res.render('about',{login:req.session.verifyLogin});
});
...
function verifyLogin(req,res,next) {
	if(req.session.verifyLogin) return next();
	res.redirect('/login');
}
